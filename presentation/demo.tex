...
\begin{frame}{Relaxing the key assumption}
  ...
  \begin{block}{Now}
    ...
    \textbf{positively correlated} with $V_I$.
  \end{block}
\end{frame}

\begin{frame}{Partial correlation structure}
  \begin{equation}
    p_H \equiv \Pr(V_U = V_H \mid V_I = V_H),
  \end{equation}
  \begin{equation}
    p_L \equiv \Pr(V_U = V_L \mid V_I = V_L).
  \end{equation}

  Marginal consistency (same prior $q$ for both):
  \begin{equation}
    (1-q)(1-p_L) = q(1-p_H).
  \end{equation}
\end{frame}
...
\begin{frame}{Three benchmarks}
  \begin{itemize}
    \item \textbf{Independence} ($p_H=q,\ p_L=1-q$) ---
          the baseline model
    \item \textbf{Full correlation} ($p_H=p_L=1$) ---
          exactly \citet{fu2006}
    \item \textbf{Partial correlation} --- the focus, on the marginal
          consistency path
  \end{itemize}
\end{frame}
...
\begin{frame}{Belief-weighted prize}
  ...
  \begin{equation}
    \Vhat(m) = m\,\Vhat(1) + (1-m)\,\Vhat(0),
  \end{equation}
  with $\Vhat(1) = V_L + p_H\,\Delta V$,
  $\;\Vhat(0) = V_H - p_L\,\Delta V$, $\;\Delta V = V_H - V_L$.
\end{frame}

\begin{frame}{Sensitivity to beliefs}
  \begin{equation}
    \dfrac{d\Vhat}{dm} = (p_H + p_L - 1)(V_H - V_L).
  \end{equation}

  \begin{itemize}
    \item Independence: derivative $= 0$, beliefs carry no value
    \item Positive correlation ($p_H+p_L>1$): a higher posterior raises
          $U$'s expected own prize
  \end{itemize}
\end{frame}
...
\begin{frame}{A numerical illustration}
  ...
  \begin{itemize}
    \item Independence: $\Vhat\equiv 1.5$,
          $x_U^{*}=\sqrt{1.5}-1\approx 0.225$
    \item Positive corr.: $\Vhat(0)=1.2$, $\Vhat(1)=1.8$;
...
\begin{frame}{The critical boundary}
  \begin{equation}
    p_L^{*}(p_H)
    = \dfrac{R\,V_H - V_L - p_H(V_H - V_L)}{R\,(V_H - V_L)}.
  \end{equation}

  Below the boundary: separation holds. Above it: the high type's
  mimicry becomes profitable.
...
\begin{frame}{Separating vs.\ pooling regions}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig6_qs_regions.pdf}

    \par\medskip{\footnotesize Critical boundary in the $(p_H,p_L)$ plane.\par}
  \end{figure}

  \centering
  \small Independence $\Rightarrow$ separating; full correlation
  $\Rightarrow$ it fails.
\end{frame}

\begin{frame}{Pooling steps in}
  When separation collapses, a pooling equilibrium supports play.
  With $\alpha \equiv 1 - \Vhat(q)/\Vhat(1)$, it exists
  \begin{equation}
    \Longleftrightarrow \quad
    \sqrt{\alpha} \;\ge\; \dfrac{V_H - V_L}{V_H + V_L}.
  \end{equation}

  At independence $\alpha=0$: no pooling --- consistent with unique
  separation there.
\end{frame}

\begin{frame}{Three equilibrium regions}
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig6_pooling_regions.pdf}

    \par\medskip{\footnotesize Separating only, coexistence, and pooling only.\par}
  \end{figure}

  \centering
  \small The independence prior sits strictly in ``separating only''.
\end{frame}
...
\begin{frame}{One continuous spectrum}
  \begin{figure}[H]
    ...
    \par\medskip{\footnotesize Effective prize ratio crosses $R$ at $t^{*}$.\par}
  \end{figure}

  \centering
  \small Along the consistency path the ratio rises monotonically.
\end{frame}
...
\begin{frame}[allowframebreaks]{Critical boundary}\label{prf:ch6}
  \begin{proof}
    Start from the high-type incentive-compatibility condition
    $\frac{V_H^2}{\bar V_H} \ge \frac{V_L\left(2V_H-V_L\right)}{\bar V_L}$
    and set it with equality. Substituting
    \[
      \bar V_H = q V_H + (1-q)V_L,
      \qquad
      \bar V_L = (1-s)V_H + s V_L,
    \]
    yields
    \[
      V_H^2\left[(1-s)V_H + s V_L\right]
      = V_L\left(2V_H-V_L\right)\left[q V_H + (1-q)V_L\right].
    \]
    Expanding the left-hand side,
    \[
      V_H^3 - s\,V_H^2\left(V_H-V_L\right)
      = V_L\left(2V_H-V_L\right)\left[q V_H + (1-q)V_L\right],
    \]
    so
    \[
      s^{*}(q) =
      \dfrac{V_H^3 - V_L\left(2V_H-V_L\right)
            \left[q V_H + (1-q)V_L\right]}
           {V_H^2\left(V_H-V_L\right)},
      \qquad
      \dfrac{d s^{*}}{d q}
      = -\,\dfrac{V_L\left(2V_H-V_L\right)}{V_H^2} < 0.
    \]
    Two benchmarks pin down the regions:
    \begin{itemize}
      \item At $(q,s)=\left(\frac12,\frac12\right)$,
        $\bar V_H=\bar V_L=\frac{V_H+V_L}{2}$, and the condition becomes
        $V_H^2 \ge V_L\left(2V_H-V_L\right)
         \iff \left(V_H-V_L\right)^2 \ge 0$, strict under $V_H>V_L$ ---
        the separating region.
      \item At $(q,s)=(1,1)$, $\bar V_H=V_H$, $\bar V_L=V_L$, and the
        condition becomes $V_H \ge 2V_H-V_L \iff V_L \ge V_H$,
        contradicting $V_H>V_L$ --- the pooling region.
    \end{itemize}
  \end{proof}
\end{frame}
