...
We consider a two-player Tullock contest between two risk-neutral players, the informed player ($I$) and the uninformed player ($U$). The informed player privately observes the realization of his own prize value before the effort stage; the uninformed player knows only the prior distribution $G$ of that value and does not observe its realization. The two players compete over whether a policy is implemented; the winner receives his own prize and the loser receives zero. If $I$ wins, he obtains $V_I > 0$; if $U$ wins, $U$ obtains $V_U equiv V > 0$. The contest success function follows the proportional-effort form of #c("tullock1980"), and the endogenous timing structure follows the action-commitment framework of #c("hamilton1990") and #c("fu2006"), in which the two players first decide when to move and then choose their efforts.

The model contains two layers of asymmetry. First, information is asymmetric across players: $U$ knows the prior distribution $G$ of $V_I$ but does not observe its realization before the time of action; $I$ has not yet observed his type at stage 0 either, and observes $V_I$ privately only after the timing decision is made. Second, the prizes themselves are asymmetric: $I$ receives $V_I$ upon winning and $U$ receives $V$, with no requirement that the two be equal. Consequently, the stage-0 timing decision is based on ex ante expected payoffs; once the effort subgame begins, $I$'s strategy can be conditioned on the realized $V_I$, whereas $U$ can react only to observable actions.

== Information structure <subsec-model-info>

At stage 0, $I$ and $U$ simultaneously choose when to act, with $I$ not yet having observed his type. Once the timing is fixed, nature draws $V_I$ from the prior distribution $G$; $I$ observes the realization privately, while $U$ does not. The two players then enter the effort subgame at the times committed to in stage 0.

== Timeline of the contest <subsec-model-timeline>

Following #c("fu2006"), the stage-0 choice of action time induces three regimes that govern the subsequent contest. If $I$ and $U$ choose the same period, both players choose efforts simultaneously in that period; we call this the $S S$ regime. If $U$ chooses period 1 and $I$ chooses period 2, $U$ commits first and $I$ chooses after observing $x_U$; we call this the $U I$ regime. If $I$ chooses period 1 and $U$ chooses period 2, $I$ commits first and $U$ chooses after observing $x_I$; we call this the $I U$ regime.

At stage 0, $I$ and $U$ simultaneously decide whether to move in period 1 or period 2, and the choice has commitment value. After the timing is determined, nature draws $V_I$ from $G$; $I$ privately observes the realization as soon as it is drawn, while $U$ does not. The two players then enter the effort subgame at the times chosen at stage 0. If either player has already moved in period 1, the effort is publicly observable in period 2, so the period-2 player can condition his effort on the first mover's choice.
...
Imposing non-negativity, these are the interior best responses; in boundary cases the relevant best response is the maximum of zero and the expressions above. Crucially, $U$'s best response depends only on $x_I$ and $V$ and contains no posterior belief about $V_I$. This is the principal departure from #c("fu2006") and is the central reason why the $I U$ subgame admits a unique separating equilibrium below.
