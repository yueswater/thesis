...
為方便後續比較，此處預先定義 $Xi_U equiv pi_U^(U I) - pi_U^(I U)$，其正式推導見後文引理；讀者可先將其理解為不知情者對 $U I$ 與 $I U$ 兩制度的報酬差額。
...
== 混同均衡的排除 <subsec-pooling-analysis>
...
這一分配無關性的分離結論，是本文對 #c("fu2006") 的主要修正：後者的混同均衡成立仰賴「$U$ 的最佳反應對 $I$ 的類型敏感」這一特定的經濟位置假設；本文揭示，一旦放寬此假設，混同均衡便從 $I U$ 子賽局中消失，分離成為競賽結構本身所保證的結論，而非特定信念精煉的產物。
...
此外，#headingref(<sec-analysis>)證明 $I U$ 子賽局僅存在唯一的分離均衡——任意真實型別與偽裝型別之間的事後報酬差距恰為 $(v - s)^2 / (4 V) > 0$，導致任何混同或半分離配置都違反誘因相容條件。在傳統訊號賽局中，分離均衡能否成立通常與型別分配的尾部、單調概似比性質等假設密切相關（例如 #c("chosobel1990"); #c("riley1979")）#footnote[#c("chosobel1990") 在連續型別賽局中刻畫分離均衡的成立條件，指出單調概似比性質是關鍵假設；本文的兩型別架構雖無需此條件，但其分離結論的不依賴分配性質，正是此文獻背景下的額外貢獻。]，但在#headingref(<sec-analysis>)的設定下這些限制全部消失，原因正是不知情者的最佳反應方程不依賴對 $V_I$ 的後驗信念。最後，#headingref(<sec-analysis>)對福利進行了獨立分析，指出三種制度下的總福利排序與均衡排序之間沒有固定的單調對應；若欲單憑特定均衡型態逕行斷定其福利效果最優，在 Tullock 競賽的部分參數區間內將會失效。

在進入#headingref(<sec-extension>)的延伸分析之前，有必要先釐清第 3–5 章的基準模型與第 6 章相關結構之間的數學銜接。基準模型中，不知情者的獎酬被設定為公開常數 $V$；而在第 6 章引入相關參數 $rho$ 後，當 $rho = 0$ 時，由 @eq-ext-vhat-derivative 知 $(d hat(V)_U (m)) \/ (d m) = rho Delta V = 0$，有效獎酬對所有後驗信念 $m in [0,1]$ 恆等於先驗加權值
$
  hat(V)_U (q) = q V_H + (1-q) V_L.
$
此時不知情者的最佳反應退化為
$
  x_U^* (x_I, q) = sqrt(x_I hat(V)_U (q)) - x_I,
$
與第 3.3 節基準最佳反應式在 $V = hat(V)_U (q)$ 時完全相同。因此，*$rho = 0$ 的統計獨立情形，等價於將基準模型的公開常數 $V$ 替換為先驗加權有效獎酬 $hat(V)_U (q)$*，第 5 章所有均衡報酬公式只需以 $hat(V)_U (q)$ 代入 $V$ 即可對應至第 6 章 $rho = 0$ 的特例。第 6 章沿 $rho in (0,1]$ 展開的分析，可理解為對此基準常數的連續擾動：相關性的引入使不知情者的有效獎酬從 $hat(V)_U (q)$ 出發，依信念方向以斜率 $rho Delta V$ 作比例調整，訊號通道隨之逐步開啟。
