...
#lemma(title: [第 0 階段最佳反應])[
  第 0 階段的最佳反應由#headingref(<subsec-stage0-analysis>)所定義的四個單邊偏離差額決定：

  1. 若不知情者選 $F$，則知情者依 $Delta_I^(U I)$ 正/負選 $S$/$F$，等號無異。
  2. 若不知情者選 $S$，則知情者依 $Delta_I^(I U)$ 正/負選 $F$/$S$，等號無異。
  3. 若知情者選 $F$，則不知情者依 $Delta_U^(S S)$ 正/負選 $F$/$S$，等號無異。
  4. 若知情者選 $S$，因 $Delta_U^(U I) >= 0$，不知情者弱偏好 $F$（僅 $V lambda = 1$ 時無異）。
] <subsec-stage0-br>
...
#theorem(title: [純策略內生時序均衡])[
  ...
  此外，$Delta_U^(U I) = 0$ 若且唯若 $V lambda = 1$，故在一般情況 $V lambda != 1$ 下，延後同時行動 $(S,S)$ 不可能是純策略均衡。
] <subsec-stage0-ne>
...
#lemma(title: [型別層級的事後分解])[
  雖然第 0 階段的制度選擇以事前報酬為基礎，型別層級的報酬差仍可用來分解知情者的時序誘因。對任一實現型別 $v$，令
...
換言之，分配假設將直接決定 $R(v)$ 在砥柱集合上的權重配置與符號結構，最終主導單邊偏離差額 $Delta_I^(I U)$ 的正負方向。下述將分別藉由「兩點分配」與「均勻分配」來闡明此一對應機制：前者著眼於兩個離散型別點的符號組合與機率權重所產生的淨效應；後者則聚焦於砥柱集合內部是否存在 $R(v)=0$ 的臨界點，以及該臨界點如何將連續的型別空間劃分為不同的偏好區塊。
...
#lemma(title: [兩點分配下的符號判準])[
  在兩點分配 $V_I in {V_L, V_H}$ 下，令 $0 < V_L < V_H$。
...
如#ref(<fig-ch5-timing-regions>)所示，在固定 $V=1$ 的兩點分配下，$(mu, delta)$ 會把參數空間劃分為不同的符號區域；這些區域對應 $Delta_I^(I U)$ 與 $Delta_U$ 的正負組合，因而直接對應到第 0 階段的制度誘因。

#figure(
  image("../../../figures/fig5_regions.pdf", width: 130%),
  caption: [兩點分配在 $(mu, delta)$ 平面上的 $(Delta_I^(I U), Delta_U)$ 符號區域。],
  supplement: [圖],
...
#lemma(title: [均勻分配下之內部臨界點與型別劃分])[
  給定型別服從均勻分配，則：

  + 若區間端點滿足 $R(mu - delta) dot R(mu + delta) < 0$，則砥柱集合內必定存在至少一內部臨界值 $v^* in (mu - delta, mu + delta)$ 滿足 $R(v^*) = 0$。該臨界點會將型別空間截然劃分為偏好先動（$cal(F)$）與偏好後動（$cal(W)$）兩區域。

  + 若 $R(mu - delta) dot R(mu + delta) > 0$ 且方程式 $R(v)=0$ 於砥柱集合內無實根，則全體型別皆具備一致之事後偏好（全局偏好先動或全局偏好後動）。
...
此外，圖中虛線代表的是參數的可行邊界 (即 $delta = mu$)；位於該虛線上方之區域將導致型別下界 $mu - delta <= 0$，從而違反本文對於砥柱集合恆正之基本假設。本圖所刻畫之可行參數範圍，可觀察到未出現全體型別皆偏好後動的參數區域。換言之，在給定的均勻分配參數族中，實質上僅能觀察到「全局偏好先動」以及「存在內部臨界點、型別偏好產生分歧」兩種結構。最後，隨著公開獎酬價值 $V$ 的逐步推升，後者所對應的淺色偏好分歧區域亦將隨之顯著擴張，隱含在更大的可行參數集合中，型別間對時序制度的事後排序不再一致；因此，第 0 階段的制度結果將更依賴型別分布的細部結構與臨界點位置，而難以由單一平均比較準則加以概括。

== 混合均衡分析 <subsec-pooling-analysis>

在三種制度中，只有 $I U$ 可能出現「先動行為是否傳遞型別訊號」之問題：知情者先動，再由不知情者觀察 $x_I$ 後反應。相對地，$S S$ 為同時行動，$U I$ 為不知情者先動，兩者都不是由知情者向不知情者傳遞訊號的子賽局。
...
只依賴觀察到的投入 $x_I$ 與自身獎酬 $V$，不依賴對 $V_I$ 的後驗信念。因此，知情者的投入雖可被觀察，但不會透過「信念更新」改變不知情者的最適反應，隱含在 $I U$ 子賽局中，訊號通道雖存在於資訊結構層次，卻不影響行為方程；均衡比較回到投入與報酬本身。

#lemma(title: [不知情者的直接時序偏好])[
  不知情者在兩個序列制度之間的直接偏好可寫為
  ...
  若 $Delta_U >= 0$，則不知情者在直接比較 $U I$ 與 $I U$ 時弱偏好自己先動；若 $Delta_U < 0$，則偏好後動。此量有助於排序兩個序列制度，但它不是 $I U$ 是否為 Nash 均衡的偏離條件；對 $I U$ 而言，關鍵是 $Delta_U^(S S) <= 0$。
]
...
#proof[
  ...
  $
    ...
    - (4 V^2 - 4 V mu + EE[V_I^2]) / (4 V).
  $
  故 $Delta_U$ 的符號直接決定不知情者在 $U I$ 與 $I U$ 的制度排序。又因第 0 階段對 $I U$ 的偏離是與 $S S$ 比較，故其均衡條件仍為 $Delta_U^(S S) <= 0$，而非 $Delta_U$。
]

上述引理區分了「制度排序」與「偏離誘因」：$Delta_U$ 只刻畫不知情者對 $U I$ 與 $I U$ 的直接排序，並不等同於策略式賽局中的偏離條件。因此，均衡分析仍須回到 $Delta_U^(S S)$ 並與其他單邊差額聯合判斷。

在此基礎上，本文與 #c("fu2006") 的主要差異可表述為下列定理。

#theorem(title: [$I U$ 子賽局的唯一分離均衡])[
  在本文模型下，$I U$ 子賽局的完美貝氏均衡是唯一分離均衡；混合均衡與半分離均衡皆無法成立。
] <thm-iu-no-pooling>
...
#proof[
  ...
  因此，每一型別都嚴格偏好自己的分離行動，任何混合或半分離配置都違反誘因相容。又因分離行動在#headingref(<sec-equilibria>)已構成完美貝氏均衡，故 $I U$ 子賽局的完美貝氏均衡必為唯一分離均衡。
]

== 福利分析 <subsec-welfare>

#headingref(<subsec-stage0-analysis>)已完整刻畫第 0 階段的均衡：哪一個時序組合可由純策略 Nash 均衡支持，端視四個單邊偏離差額 $Delta_I^(U I)$、$Delta_I^(I U)$、$Delta_U^(S S)$ 與 $Delta_U^(U I)$ 的正負而定。然而「均衡」與「社會福利」是兩個不同層面的問題：均衡關心的是給定對手策略，任一參與者是否願意單邊偏離；福利關心的
則是雙方報酬之和的整體大小。在 Tullock 競賽的結構下，此一區別實益在於——獎酬本身固定，雙方投入越多，「白費」於競賽的資源就越多，社會剩餘就越少。因此本節進行的福利分析，等同於比較三種制度下的「總投入」：投入越低，越能將資源保留給最終得勝者，而非耗散在競賽過程中。

回到模型，三種制度產生的均衡投入結構各有其特徵——$S S$ 制度為同時行動，雙方在不確定對方真實獎酬的情況下選擇投入，期望投入水平由 $kappa$、$lambda$ 決定；$U I$ 制度為不知情者先動，因不知情者無法觀察 $V_I$，必須以先驗代替後驗來計算投入，事後知情者才依實現型別做出反應；$I U$ 制度則為知情者先動，每一型別在不知情者觀察後並反應選擇投入。三者在型別風險如何被吸收進均衡投入這一點上彼此不同，進而造成總效率的差異。值得注意的是，福利排序對先驗 $q$、砥柱集合的分散度 $delta$ 以及獎酬水準 $V$ 等都有非單調的依賴關係，故在一般參數空間下，不存在「某一制度永遠最優」之結論。
...
從三式相減的結果可以看出，福利的排序與均衡的制度排序之間並不存在絕對的對應關係。例如，即使某些參數設定下 $I U$ 被確定為均衡制度，也無法由此直接推斷 $I U$ 的總福利一定高於 $S S$；同樣地，反之亦然。簡言之，「哪一個制度在均衡中得以實現」與「哪一個制度能帶來較高的總福利」，這是兩個彼此分立的問題，前者不必然推導出後者。這個結果與 Tullock 競賽相關文獻中所提出的發現是一致的：優勢者的先行承諾可能會導致更高程度的投入與資源耗散，而在內生時序條件下經常出現的「弱者先行、強者後行」安排，也不一定符合總收益最大化或設計者目標的最優解。雖然不同文獻中採用的福利評估標準可能有所差異，但這些研究皆表明：均衡時序的決定與福利效應的評估需要劃分為獨立的考量範疇 #ca("dixit1987"), #cy("dixit1987")；#ca("baikshogren1992"), #cy("baikshogren1992")；#ca("leininger1993"), #cy("leininger1993")；#ca("nitzan1994"), #cy("nitzan1994")；#ca("baik2013"), #cy("baik2013")；#ca("protopappas2023"), #cy("protopappas2023")。故在本文的分析框架下，對不同制度進行比較時，僅僅考慮哪些制度能實現均衡仍稍顯不足，必須進一步在特定參數條件下檢驗福利效果，從而確保準確性與穩健性。

== 小結 <subsec-ch5-summary>

#headingref(<sec-analysis>)將#headingref(<sec-equilibria>)求得的三個子賽局均衡報酬代回第 0 階段的策略式賽局，並透過#headingref(<subsec-stage0-analysis>)完整刻畫了內生時序均衡。核心工具是四個單邊偏離差額 $Delta_I^(U I)$、$Delta_I^(I U)$、$Delta_U^(S S)$ 與 $Delta_U^(U I)$；前三者的正負組合直接決定哪一個時序組合可由純策略 Nash 均衡決定，而第四個差額則因 $Delta_U^(U I) >= 0$ 恆成立，條件會退化成 $V lambda = 1$ 的臨界值——隱含「延後同時行動」在一般參數下並不會構成均衡。在此基礎上，#headingref(<sec-analysis>)引入 $R(v)$ 函數，將知情者的事後比較拆分至型別層次：

- 在兩點分配下，$R(V_L)$ 與 $R(V_H)$ 的符號組合與機率加權即決定 $Delta_I^(I U)$ 的方向。
- 在均勻分配下，臨界點 $v^*$ 是否落在砥柱集合內決定型別空間是否一分為二。

此外，#headingref(<sec-analysis>)證明 $I U$ 子賽局僅存在唯一的分離均衡——任意真實型別與偽裝型別之間的事後報酬差距恰為 $(v - s)^2 / (4 V) > 0$，導致任何混合或半分離配置都違反誘因相容條件。在傳統訊號賽局中，分離均衡能否成立通常與型別分配的尾部、單調概似比性質等假設密切相關，但在#headingref(<sec-analysis>)的設定下這些限制全部消失，原因正是不知情者的最佳反應方程不依賴對 $V_I$ 的後驗信念。最後，#headingref(<sec-analysis>)對福利進行了獨立分析，指出三種制度下的總福利排序與均衡排序之間沒有固定的單調對應；若欲單憑特定均衡型態逕行斷定其福利效果最優，在 Tullock 競賽的部分參數區間內將會失效。

上述分離結果之所以展現出分配無關性之結果，源於#headingref(<sec-analysis>)的一個關鍵假設：不知情者的獎酬價值為公開常數 $V$。當 $V_U$ 為已知常數時，不知情者的最佳反應函數僅取決於觀察到的投入量，與其對 $V_I$ 的後驗信念無關。換言之，雖然知情者的先動行為在資訊結構上是可觀察的，卻無法透過信念更新來干預不知情者的決策行為，導致訊號傳遞機制無從發揮作用——這也正是為何#headingref(<sec-analysis>)的分離均衡恆成立，而 #c("fu2006") 中須仰賴特定分配假設才能建立的混合均衡，在本文框架下會被完全排除。
