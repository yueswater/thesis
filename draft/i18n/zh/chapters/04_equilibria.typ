...
$I U$ 子賽局的關鍵是分離，而非混合。每一型別都嚴格偏好自己的分離投入，不會模仿其他型別；因此，混合均衡與半分離均衡皆被排除。相較於 #c("fu2006")，本文的 $I U$ 子賽局沒有因信念而產生的均衡選擇問題；訊號雖然存在於觀察層次，卻不會改變不知情者的最佳反應。

若進一步聚焦於兩點型別 $V_I in {V_L, V_H}$，且先驗為 $Pr(V_I=V_H)=q$、$Pr(V_I=V_L)=1-q$，則高型別與低型別的雙向模仿差額分別為
