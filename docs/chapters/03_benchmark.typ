...
本章以#term("獨立同分配", english: "independent and identically distributed")的 benchmark 作為比較起點。目的不在重述 #c("fu2006") 的結論，而在隔離其成立所依賴的資訊結構：當 $V_I$ 與 $V_U$ 相互獨立時，知情者的投入雖仍可能揭示型別，不知情者對自身獎酬的評估卻不隨之改變。據此，本章先單獨檢視時序本身對均衡結果的影響，作為後續部分相關模型的比較基準。

== 問題意識：#c("fu2006") 結論的成立機制 <subsec-question>

本章關心的問題是：#c("fu2006") 的後動結果，究竟來自競賽時序本身，抑或來自資訊相關性所支撐的#impt[訊號—信念—反應]機制？獨立同分配的 benchmark 正是為了隔離後者而設。
...
本文沿用 #c("hamilton1990") 與 #c("fu2006") 的兩期承諾架構。若雙方選擇於同一期投入，則形成同時行動的 SS 時序；若不知情者先動、知情者後動，則形成 UI 時序；若知情者先動、不知情者後動，則形成 IU 時序。時序確定後，雙方進入對應的投入子賽局，並依據各自掌握的資訊進行決策。在此 benchmark 中，即使不知情者可由知情者的先行投入推論其型別，該推論亦不改變不知情者對自身獎酬的條件期望。例如，當知情者為低型別時，有

$
  EE[V_U|V_I = v_L] = EE[V_U].
$

因此，後驗信念雖然仍可更新，不知情者的後驗期望獎酬與最佳反應卻維持不變。此一 benchmark 的作用，即在提供後續時序比較的共同出發點。

== 基準模型的理論意涵 <subsec-iid-implications>

在獨立同分配基準模型中，觀察知情者投入至多改變對其型別的判斷，卻不改變不知情者對自身獎酬的評估。對任意由知情者型別或投入訊號所生成、且不含 $V_U$ 額外資訊之事件 $A$，皆有
$
  EE[V_U|A] = EE[V_U] = overline(v),
  quad
  overline(v) = q v_H + (1-q)v_L.
$
因此，$widetilde(V)_U(mu)$ 在此退化為常數 $overline(v)$，不知情者於任一路徑上的最佳反應均不依賴後驗信念；後續相關模型中額外出現的策略效果，皆應相對於此 benchmark 理解。

== 子賽局均衡 <subsec-bench-subgame-equilibria>

在獨立同分配基準模型下，$widetilde(V)_U (mu) = overline(v)$ 對所有後驗信念 $mu$ 成立。因此，不知情者在三個子賽局中的有效獎酬均退化為無條件期望值 $overline(v)$，均衡結構因而具有特別簡潔的形式。以下分別刻畫 SS、UI 與 IU 三個子賽局之均衡。
