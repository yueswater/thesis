...
#stmtref(<prop-main-endogenous-timing-pure>, "定理", "theorem-counter") 顯示，第 0 階段的純策略制度選擇可完全化約為四個單邊偏離差額的正負比較。內生時序均衡並非直接由先動或後動本身決定，而是取決於某一方在固定對手時點選擇下，是否能藉由改變自身行動時序提升制度報酬。故時序均衡的核心，不在於個別制度的絕對報酬高低，而在於制度之間的局部偏離誘因如何排序；此亦構成後續分析相關性如何牽動時序翻轉的出發點。
...
#stmtref(<prop-main-timing-u-prefers-ui>, "引理", "lemma-counter") 的意義在於，不知情者面對後動的知情者時，始終弱偏好以先動承諾將制度推向 UI。換言之，相關性雖會改變三種制度下的事前報酬水準，但不會推翻 UI 相對於 SS 的這一方向性比較。故第 0 階段時序翻轉的主要來源，並不在於不知情者是否放棄先動，而在於知情者對先動揭露效果的評價如何隨 $rho$ 改變。
...
由@fig-main-timing-preference 可見，在低相關區域，知情者先動所造成的資訊揭露成本有限，故 IU 制度下之事前報酬高於 SS；然而，隨著 $rho$ 上升，知情者型別對不知情者自身獎酬的推論價值逐漸提高，先動揭露所引發的競爭反應因而加劇，使知情者的時序偏好由 IU 轉向 SS。當 $rho$ 進一步超過 $rho^*(q)$ 後，高型別模仿低型別的誘因已無法由未扭曲分離配置排除，故未扭曲分離均衡失效。
...
就不知情者而言，由#stmtref(<prop-main-timing-u-prefers-ui>, "引理", "lemma-counter")，$Delta_U^(U I) >= 0$ 對任意 $rho$ 成立，等號條件為 $tilde(B)=1$。#stmtref(<prop-main-comparison-ab-monotone>, "引理", "lemma-counter") 指出 $tilde(B)$ 單調遞減且僅在 $rho=1$ 時降至 $1$，故在 $rho < 1$ 的全部區間，不知情者對 UI 制度的先動偏好嚴格為正。換言之，不論相關性如何改變，不知情者始終願意在知情者後動的制度承諾下搶先行動，此一偏好不因 $rho$ 上升而翻轉。
