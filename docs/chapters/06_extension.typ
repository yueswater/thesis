...
在訊號賽局文獻中，低型別的最低成本分離行動——亦即使高型別誘因相容條件恰好綁住的最小訊號量——被稱為 Riley 結果（#c("riley1979")）。D1 準則（#c("cho1987")）在一般單調訊號賽局中唯一選出此結果（見命題「D1 準則唯一選出 Riley 結果」）。

#proposition(title: [最低成本扭曲分離（Riley 結果，#c("riley1979")）])[
...
2. 在完全相關端點 $rho = 1$，有 $hat(V)_U(1) = V_H$、$hat(V)_U(0) = V_L$，故比值為 $V_H \/ V_L > R$；未扭曲分離失敗（此為 $rho > rho^*$ 的情形；Riley 扭曲分離的完整刻畫及區域 C' 的定義見#headingref(<subsec-ext-timing>)，在基準參數 $V_H = 2 V_L$ 下 $rho^(**) = 1$，$rho = 1$ 對應區域 C' 的右端點）。
...
為使上述比較靜態結論於相關參數軸上獲得直觀呈現，圖 @fig-ch6-pooling-regions[]將#headingref(<subsec-ext-boundary>)推導之分離臨界相關度 $rho^*$ 與本節得出之混同臨界相關度 $rho_P$ 並陳於同一條 $rho in [0, 1]$ 數軸上（取基準 $V_H = 2$、$V_L = 1$、$q = 0.5$，故 $rho_P = 0.375 < rho^* approx 0.43$）。由於混同臨界 $rho_P$ 恆低於分離臨界 $rho^*$，整段相關性光譜可細分為四個經濟意涵相異之子區間（完整刻畫見#headingref(<subsec-ext-timing>)命題「$I U$ 子賽局的全均衡刻畫」）：
...
#headingref(<subsec-ext-pooling>)所建構之混同均衡，其存在性仰賴一條極強且相當任意的脫軌信念假設：$m(x != x_P) = 1$。由於本章招牌結論之並存區 (Region C) 整個建立在混同均衡存在之上，若混同無法通過標準的信念精煉，則區域 C 恐僅是寬鬆脫軌信念的產物，而非穩健的賽局現象。本節據此對混同均衡施以 #c("cho1987") 之直覺準則 (Intuitive Criterion)，檢驗其是否存活。
...
#proposition(title: [混同均衡通過直覺準則])[
  每一個純策略混同均衡均通過 #c("cho1987") 之直覺準則，亦通過更強之 D1 準則。
] <prop-ext-refinement>
...
#proof[
  ...
  *步驟一：剔除 $x_L'' < x_L^"Riley"$ 的均衡。* 考慮任意以 $x_L'' < x_L^"Riley"$ 為低型別均衡投入的候選分離均衡，取脫離路徑投入 $tilde(x) in (x_L'', x_L^"Riley")$（即 $tilde(z) in (sqrt(x_L''), z_H^-)$）。由於 $tilde(z) < z_H^-$，二次式 $(*)$ 在 $tilde(z)$ 處嚴格為正：
  ...
  *步驟二：Riley 結果存活。* 對任意脫離路徑投入 $x' != x_L^"Riley"$，分三個區間討論：
  ...
  三種情形皆不構成有效偏離，Riley 結果存活。綜上，D1 唯一選出 $x_L^"Riley" = (z_H^-)^2$。
]
...
綜合#headingref(<subsec-ext-boundary>)至#headingref(<subsec-ext-refinement>)之分析，現在可以對 $I U$ 子賽局的純策略均衡給出窮舉式刻畫，並直接呈現本文的核心光譜主張：#c("fu2006") 的設定（$rho = 1$）與本文基準（$rho = 0$）分別是此光譜的兩個端點，而下述命題所刻畫的四個區域，則是端點之間所有可能均衡型態的完整連續。
...
在揭露型態之外，相關性更透過子賽局報酬牽動第 0 階段的時序選擇。回顧#headingref(<subsec-pooling-analysis>)，#c("fu2006") 與本文基準模型對「知情者是否願意先動」給出截然不同的答案：在 #c("fu2006") 的完全相關設定下，知情者先動會經由訊號通道洩露評價、侵蝕資訊租金，因此知情者嚴格偏好後動、由不知情者先動 ($U I$)；而在本文 $rho = 0$ 的獨立基準下，投入層次的訊號通道關閉，知情者的時序偏好回歸純粹的事前期望報酬比較，$I U$ 在部分參數下反而可被支持。本章的單參數架構恰好把這兩種對立的時序結論連成一條連續光譜。
...
$
不等式成立的根源在於 $S S$ 允許型別特定的投入調整，而混同制度強制投入同質化，抹去知情者依私有訊息微調的空間。在基準參數 $V_H = 2$、$V_L = 1$、$q = 0.5$ 下可數值確認：$tilde(pi)_I^(S S) approx 0.408 > 0.375 = mu^2 \/ (4 hat(V)_U (q))$，故混同路徑上 $Delta_I^(I U)(1) < 0$ 成立。
...
在完全相關端點，$hat(V)_U (1) = V_H$，$I U$ 子賽局精確對應 #c("fu2006") 的原始設定：訊號通道全開，資訊租金遭侵蝕，#c("fu2006") 已嚴格證明此時知情者偏好後動。在基準參數下可數值確認，取 $I U$ 子賽局對知情者最有利的均衡（Riley 或混同，各自算出後取較大值），均有 $tilde(pi)_I^(I U)|_(rho=1) < tilde(pi)_I^(S S)|_(rho=1)$，即 $Delta_I^(I U)(1) < 0$，與 #c("fu2006") 之結論吻合。

*綜合小結。*
時序翻轉臨界 $rho^dagger$ 的存在唯一性已由命題 8 在 $rho in [0, rho^*]$ 的嚴格單調遞減論證確立；本備註進一步確認在 $rho in (rho^*, 1]$，知情者偏好後動（$Delta_I^(I U) <= 0$）的結論持續成立。兩段論述合觀，完整刻畫了時序偏好沿相關性光譜由正轉負的完整過渡，而 #c("fu2006") 的後動結論正是此光譜在 $rho = 1$ 極端端點的退化特例。

上述之時序偏好命題補上了本章「Fu 為光譜端點」主張的最後一環：先前各節說明的是 $rho$ 如何決定 $I U$ 子賽局內部的揭露型態 (分離或混同)，本節則進一步把相關性接回第 0 階段，說明 $rho$ 如何決定知情者是否願意先動。兩者合觀，方完整呈現相關性低時仍可支持先動、相關性高時先動誘因消失的時序光譜，而 #c("fu2006") 正是此光譜在 $rho = 1$ 的端點。
