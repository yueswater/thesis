...
def _boundary(p_h: np.ndarray, v_h: float, V_L: float) -> np.ndarray:
...
def render_qs_regions(v_h: float = 2.0, V_L: float = 1.0) -> None:
    ...
    region = np.where(p_L_grid <= _boundary(p_h_grid, v_h, V_L), 0, 1)

    fig, ax = plt.subplots(figsize=(6.2, 5.1))
    ax.contourf(p_h_grid, p_L_grid, region, levels=[-0.5, 0.5, 1.5],
                colors=["#E8E8E8", "#8D8D8D"], antialiased=False, zorder=1)

    p_h_line = np.linspace(0.0, 1.0, 600)
    p_L_line = _boundary(p_h_line, v_h, V_L)
    mask = (p_L_line >= 0.0) & (p_L_line <= 1.0)
    ax.plot(p_h_line[mask], p_L_line[mask], color="black", linewidth=0.9, zorder=4)

    for label, q, offset in (("A", 0.3, (0.020, 0.020)), ("B", 0.5, (0.020, 0.020)), ("C", 0.7, (0.020, 0.020))):
        ax.plot(q, 1.0 - q, marker="o", color="black", markersize=4.8, zorder=5)
        ax.text(q + offset[0], 1.0 - q + offset[1], label, fontsize=9, color="black", fontweight="bold")

    ax.plot(1.0, 1.0, marker="s", color="black", markersize=6.5, zorder=5)
    ax.annotate("$(1,1)$\nPerfect Correlation", xy=(1.0, 1.0), xytext=(0.62, 0.86),
    ...
    style_axes(ax, r"$p_H=\Pr(V_U=V_H\mid V_I=V_H)$", r"$p_\ell=\Pr(V_U=V_\ell\mid V_I=V_\ell)$")
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ...
    ax.legend(handles=legend_items, loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8,
              frameon=True, facecolor="white", edgecolor="black", fancybox=False)
    fig.tight_layout()
    fig.savefig(FIGURES_DIR / "fig6_qs_regions.png", dpi=300, bbox_inches="tight")
    plt.close(fig)
